...
export const createThought = async (req, res) => {
    try {
        ...
        const user = await User.findOneAndUpdate({ username: req.body.username }, { $addToSet: { thoughts: thought._id } }, { runValidators: true, new: true });
        if (!user) {
...
export const addReaction = async (req, res) => {
    try {
        ...
        const user = await User.findOneAndUpdate({ username: req.body.username }, { $addToSet: { reactions: req.body } }, { runValidators: true, new: true });
        if (!user) {
...
export const removeReaction = async (req, res) => {
    try {
        ...
        const user = await User.findOneAndUpdate({ thoughts: req.params.thoughtId }, { $pull: { thoughts: req.params.thoughtId } }, { runValidators: true, new: true });
        if (!user) {
